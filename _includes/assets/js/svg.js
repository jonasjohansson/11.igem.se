...
function init() {
    ...
    setInterval(function() {
        var index = getRandomInt(0, SBOL.shapes.length - 1);
        addShape(index);
    }, 500);
    ...
    function addShape(index) {
        let svg = getShape(index);
        var paths = svg.querySelectorAll('path');

        var color = SBOL.colors[getRandomInt(0, SBOL.colors.length - 1)];
...
function getPos() {
    var x = SBOL.bounds[0] + 1;
    while (x > SBOL.bounds[0] && x < SBOL.bounds[1]) {
        x = getRandomInt(SBOL.offset[0], SBOL.offset[1]);
    }
    y = getRandomInt(SBOL.offset[0], SBOL.offset[1]);
    return [x, y];
}
